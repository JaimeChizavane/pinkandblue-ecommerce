...
class ProprietarioModel extends Model
{
	...
	public function listarProprietarios($post)
	{
		$this->query("DELETE FROM proprietarios WHERE (proprietarios.nome IS NULL OR proprietarios.nome = '') ");
		$this->execute();
		...
		{
			$where .= " AND proprietarios.nome LIKE '%".$post["search"]["value"]."%' ";
		}
	...
	{
		$this->query('SELECT * FROM proprietarios '.$cond.'');
		$rows = $this->resultSet();
		return count($rows);
	}
	...
	public function removerProprietarios($post)
	{
		$id		= implode(", ",$post["id"]);
		$this->query("DELETE FROM proprietarios WHERE id IN({$id})");
		$this->execute();
		echo json_encode(array(
			"status"	=> true
		));

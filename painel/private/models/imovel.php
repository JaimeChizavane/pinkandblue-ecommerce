...
{	
	...
	{
		$this->query('DELETE FROM imoveis WHERE (nome is null)');
		$this->execute();
		$this->query('SELECT * FROM imoveis ORDER BY id DESC LIMIT 0,12');
		$rows = $this->resultSet();
		return $rows;
	}
	...
	public function salvar()
	{
		...
		if(isset($post["accao"]) && $post["accao"] == "salvar_imovel")
		{
			...
			$this->query('SELECT * FROM imoveis WHERE id = '.$_GET["id"]);
			$this->execute();
			$rows = $this->single();
			return $rows;
		}
	...
	{
		$data = date("Y-m-d");
		$this->query('INSERT INTO imoveis SET 
				nome 			=:nome, 
				descricao 		=:descricao, 
				preco 			=:preco, 
				quartos 		=:quartos, 
		...
		if($lastInsertId) 
			echo json_encode(array(
				"status"=>"success",
				"novo_imovel"=>$lastInsertId
		));	
		exit;	
	}
		
	//funcao responsavel por retornar a galeria do imovel
	public function retornaGaleria()
	{
		$post = filter_input_array(INPUT_POST, FILTER_SANITIZE_STRING);
		$this->query("SELECT * FROM galeria_imovel WHERE imovel_id = :imovel_id");
		$this->bind(':imovel_id',$post['id']);
		$this->execute();
		$output		= '';		
		$galeria_imovel = $this->resultSet();
		foreach($galeria_imovel as $galeria):
		$output	.= '<div class="dev-col s6 m3 l3" style="padding:8px;position:relative;" id="foto_galeria_'.$galeria['id'].'">
				<img class="dev-border dev-padding-small dev-round" src="'.ROOT_URL.'tim.php?src='.ROOT_URL.'private/uploads/imoveis/'.$galeria['nome'].'&w=200&h=200" style="max-width:100%;">
				<span class="remove_icon_foto_galeria" data-url="private/uploads/imoveis/'.$galeria['nome'].'" data-id="'.$galeria['id'].'"><i class="fa fa-times"></i></span>
			</div>';
		endforeach;
	...
	public function retornafoto()
	{
		$post 	= filter_input_array(INPUT_POST, FILTER_SANITIZE_STRING);
		$output = '';
		$this->query("SELECT * FROM imoveis WHERE id = :id");
		$this->bind(':id',$_POST['id']);
		$row = $this->single();
		if(!is_null($row['foto'])):
			$output .= '<div class="dev-col s12 m6 l6" id="foto_'.$post['id'].'" style="position:relative;">';
			$output .= '<img class="dev-border dev-padding-small dev-round-large" src="'.ROOT_URL.'tim.php?src='.ROOT_URL.'private/uploads/imoveis/'.$row['foto'].'&w=380&h=215" style="max-width:100%;width:100%;">';
			$output .= '<span class="remove_icon_foto" data-url="private/uploads/imoveis/'.$row['foto'].'" data-id="'.$row['id'].'"><i class="fa fa-times"></i></span>';
			$output .= '</div>';
			$output .= '<div style="clear:both;"></div>';			
		endif;
		echo json_encode(array(
	...
	public function uploadfoto()
	{
		...
		if(isset($post['accao']) && $post['accao'] == 'uploadfoto'){
			$this->query('SELECT foto FROM imoveis WHERE id = :id');
			$this->bind(':id',$post['id']);
			$retornafoto 	= $this->single();
			$foto 			= !empty($retornafoto['foto']) ? $retornafoto['foto'] : '';
			$dir    		= 'private/uploads/imoveis/';			
			if(!file_exists($dir)){
				mkdir($dir,0577);
			}			
			if(isset($_FILES['foto']['tmp_name']) && !empty($_FILES['foto']['tmp_name'])):
				...
				if(move_uploaded_file($_FILES['foto']['tmp_name'],$dir.$foto)){
					$this->query('UPDATE imoveis SET foto = :foto WHERE id = :id');
					$this->bind(':foto', $foto);
					$this->bind(':id', $post['id']);
					$this->execute();
					echo json_encode(array(
	...
	public function uploadGaleria()
	{
		...
		if(isset($post['accao']) && $post['accao'] == 'uploadGaleria'){
			...
			if(isset($_FILES['galeria_imovel']['tmp_name']) && !empty($_FILES['galeria_imovel']['tmp_name'])){
				$this->query('INSERT INTO galeria_imovel (nome, imovel_id) VALUES(:nome, :imovel_id)');
				$contagem = count($_FILES['galeria_imovel']['tmp_name']);	
				for($i = 0; $i < $contagem; $i++){
					$fotoTmp 	= $_FILES['galeria_imovel']['tmp_name'][$i];
					$referencia_galeria = explode(".",$_FILES['galeria_imovel']['name'][$i]);
	...
	public function removefoto()
	{
		...
		$this->query("UPDATE imoveis SET foto = null WHERE id = :id");
		$this->bind(':id',$post['id']);
		$this->execute();
		echo json_encode(array(
			"status" => "success",
	...
	public function removeGaleria()
	{
		...
		$this->query("DELETE FROM galeria_imovel WHERE id = :id");
		$this->bind(':id',$post['id']);
		$this->execute();
		echo json_encode(array(
			"status" => "success",
	...
	public function remover()
	{
		...
		{
			if($post['id'] > 0){
				$this->query("DELETE FROM imoveis WHERE id =:id");
				$this->bind(":id",$post['id']);
				$this->execute();
				echo json_encode(array(
					'status' => 'success'					
	...
	public function agrupar()
	{
		...
		if(isset($post["accao"]) && $post["accao"] == "filtrar")
		{
			...
			$this->query('SELECT * FROM imoveis '.$cond.' ORDER BY id DESC '.$limit.'');
			$this->execute();
			$data = $this->resultSet();
			foreach($data as $model){
				$last_id 	 = $model["id"];
				$output 	.= '<div class="dev-col l3 m6 dev-margin-bottom" style="padding-left:15px;padding-right:15px;" id="imoveis_'.$model["id"].'">';
				$output 	.= '<div class="dev-border dev-round-large dev-white dev-display-container">';
					$output 	.= '<div style="">';
						$output 	.= '<img src="tim.php?src='.ROOT_URL."private/uploads/imoveis/".$model["foto"].'&w=317&h=224&a=c" alt="'.$model["nome"].'" class="dev-image dev-padding-small" style="max-height:224px;width:100%;">';
					$output 	.= '</div>';
					$output 	.= '<div class="dev-padding">';
						...
						<a href="'.ROOT.'imovel/'.$model['link'].'" target="_blank" class="dev-text-blue" style="text-decoration:none;">'.$model["nome"].'</a></p>';
					...
					$output 	.= '<div class="dev-padding">
						...
						<div class="dropdown">
						  ...
						  <div class="dropdown-content">
							<a href="'.ROOT_URL."imoveis/salvar/".$model["id"].'" class="" style="border-radius:0;"><i class="fa fa-pencil"></i> Editar</a>
							<a class="j_remove_imovel" href="'.$model["id"].'" style="border-radius:0;"><i class="fa fa-trash"></i> Remover</a>
							<a class="" href="'.ROOT.'imovel/'.$model['link'].'" target="_blank" style="border-radius:0;"><i class="fa fa-search"></i> Ver</a>
						  </div>
						</div>
					</div>';
			...
			$this->query('SELECT * FROM imoveis '.$cond.' ORDER BY id DESC ');
			$this->execute();

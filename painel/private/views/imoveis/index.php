<div class="dev-container">
	<div class="dev-margin-top dev-clear">
		<div class="dev-col l3">
			<a href="#" class="dev-button dev-large dev-block dev-flat-belize-hole j_cria_novo_imovel"><i class="fa fa-plus"></i> Adicionar</a>
		</div>	
		<select class="dev-select dev-col l3 dev-border dev-large dev-round-xlarge j_select_categoria" name="categoria_id" id="categoria_id">				
			...
			<?php 
				$uteis = new Uteis;
				$uteis->query('SELECT * FROM categorias WHERE status = 1 AND tipo IN("imovel")');
				$uteis->execute();
		...
		<select class="dev-select dev-border dev-col l3 dev-large dev-round-xlarge j_select_subcategoria" name="subcategoria_id" id="subcategoria_id">
			<option value="" disabled selected>Selecciona Bairro</option>	
			<?php 
				$uteis = new Uteis;
				$uteis->query('SELECT * FROM bairros WHERE status = 1 ');
				$uteis->execute();
				$rows= $uteis->resultSet();
				foreach($rows as $bairros):
					echo '<option value="'.$bairros["id"].'">'.$bairros["nome"].'</option>';
				endforeach;
			?>
		</select>
		...
		<div class="dev-col l12 dev-margin-top">
			<input type="text" id="myInput"class="dev-round-large j_agrupar_imovel_titulo" placeholder="Pesquise Por titulo..">
		</div>	
	</div>
			...
			<div class="dev-col l3 m6 dev-margin-bottom" style="padding-left:15px;padding-right:15px;" id="imoveis_<?php echo $model["id"]?>">
				<div class="dev-border dev-round-large dev-white dev-display-container">
					...
					<div class="dev-padding">
						...
						<p style="margin:0;" class="dev-large"><i class="fa fa-money"></i> <?php echo number_format($model["preco"],2)?> <?php echo $model["moeda"]?></p>

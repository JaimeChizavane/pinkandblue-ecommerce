<div class="dev-container">	
	<?php 
		$uteis 		= new Uteis;
		$id 		= (int) $_GET['id'];
		$uteis->query('SELECT * FROM subcategorias WHERE id =:id');
		$uteis->bind(':id',$id);
		$uteis->execute();
	...
	<div class="dev-row-padding dev-margin-bottom" style="">
		<div class="dev-sidebar dev-collapse dev-border dev-grey" style="z-index:43;width:270px;" >
			<div class="dev-bar-block dev-animate-right" id="subcategorias" >
				<div class="dev-bar-item dev-border-bottom dev-white"><?php if($_GET["tipo"] == "automovel") echo "Modelos"; else echo "Subategorias";?> - Salvar</div>
				<ul class="dev-ul dev-hoverable dev-large j_detalhes_da_subcategoria">		
					<li style="padding:0;"><a href="#descricao_da_subcategoria" class="dev-bar-item dev-large">Descrição</a></li>
					<li style="padding:0;"><a href="#foto_principal_da_subcategoria" class="dev-bar-item dev-large">Foto Principal</a></li>
				</ul>	
			</div>	
		</div>
		<div class="dev-main" style="margin-left:270px;"><!--Aqui Inicia A Div para cadastro dos dados da Subcategoria-->			
			<div class="dev-border dev-white dev-padding">
				<h1 class="dev-border-bottom dev-text-grey" id="descricao_da_subcategoria" style="margin-bottom:0;">Descrição</h1>
				<form name="salva_subcategoria" class="" style="margin-top:0;margin-bottom:0;" method="POST" id="salva_subcategoria" action="<?php $_SERVER["PHP_SELF"];?>">
					<div class="dev-row-padding">
						<input type="hidden" value="<?php echo $id;?>" name="id">
						<p>
							<label class="dev-large" for="nome"><?php if($_GET["tipo"] == "automovel") echo "Modelo"; else echo "Subategoria";?></label>
							<input class="dev-input dev-border dev-large dev-round-xlarge" type="text" name="nome" id="nome" value="<?php echo $viewmodel["nome"]?>" placeholder="<?php if($_GET["tipo"] == "automovel") echo "Modelo"; else echo "Subategoria";?>" required>
						</p>
					...
					<div class="dev-row-padding" style="padding:0;">
						<div class="dev-half">
							<p>
								<label class="dev-large" for="bairro_id">
									<?php if($_GET["tipo"] == "automovel") echo "Marca"; else echo "Categoria";?>
								</label>
								<select class="dev-select dev-border dev-large dev-round-xlarge" name="categoria_id" id="categoria_id">									
									<option value="" disabled selected>Informe <?php if($_GET["tipo"] == "automovel") echo "Marca"; else echo "Categoria";?></option>
									<?php 
										$uteis = new Uteis;
										$uteis->query('SELECT * FROM categorias WHERE status = 1 AND tipo IN("'.$_GET["tipo"].'")');
										$uteis->execute();
						...
						<div class="dev-half">
							<p>
								...
								<select class="dev-select dev-border dev-large dev-round-xlarge" name="status" id="status" required>
									<option value="" disabled selected>Informe o status da Subcategoria</option>
					...
					<p class="j_subcategoria">
						<input type="submit" class="dev-button dev-flat-belize-hole j_salva_subcategoria" name="salvar_contacto" value="Salvar&nbsp; ❯">
				...
				<div class="dev-white" style="padding-top:0;">
					<h1 class="dev-border-bottom dev-text-grey" id="foto_principal_da_subcategoria" style="margin-bottom:0;margin-top:0;">Foto Principal</h1>
		...
		</div><!--Aqui Finaliza A Div para cadastro dos dados da Subcategoria-->
	</div>
</div>
<style>
	button{
...
$(document).ready(function(){
  ...
  $(".j_detalhes_da_subcategoria > li > a").on('click', function(event) {
...
var theForm = document.getElementById("salva_subcategoria");
...
<script src="<?php echo ROOT_URL;?>assets/controllers/subcategoria.js"></script>